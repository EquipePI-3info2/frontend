    ...
    <main class="container order-detail__content">
      ...
      <template v-else-if="order">
        ...
        <section class="detail-card">
          <h2>Itens</h2>
          <article v-for="item in order.items" :key="`product-${item.id}`" class="order-item">
          ...
          <article v-for="item in order.kit_items || []" :key="`kit-${item.id}`" class="order-item order-item--kit">
            <img v-if="item.kit_image" :src="item.kit_image" :alt="item.kit_name" />
            <div v-else class="order-item__kit-placeholder"><Gift :size="22" /></div>
            <div>
              <strong>{{ item.kit_name }} <small>Kit</small></strong>
              <p>{{ item.quantity }} × {{ formatCurrency(item.unit_price) }}</p>
              <span class="order-item__components">{{ kitComponents(item) }}</span>
            </div>
            <strong>{{ formatCurrency(item.subtotal) }}</strong>
          </article>
...
function kitComponents(item) {
  return (item.components || [])
    .map((component) => `${component.quantity_per_kit}× ${component.product_name}`)
    .join(' + ')
}
...
.order-item img, .order-item__kit-placeholder { width: 54px; height: 54px; border-radius: var(--radius-md); }
.order-item img { object-fit: cover; }
.order-item__kit-placeholder { display: flex; align-items: center; justify-content: center; background: #fff1e8; color: var(--color-primary-light); }
.order-item--kit { background: #fffaf7; padding: var(--space-2); border-radius: var(--radius-md); }
.order-item--kit small { color: #9a5d1c; font-size: .58rem; font-weight: 900; text-transform: uppercase; }
.order-item__components { display: block; color: var(--color-text-muted); font-size: .65rem; line-height: 1.3; margin-top: 2px; }
